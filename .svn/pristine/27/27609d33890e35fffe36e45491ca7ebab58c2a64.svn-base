...
*****************************************************************************************/
class EPOCH_EXPORT CMD5Checksum  
{
public:
	//interface functions for the RSA MD5 calculation
	static GUID GetMD5(BYTE* pBuf, UINT nLength);
	static GUID GetMD5(const std::vector<BYTE>& Buf);
	static GUID GetMD5(LPCTSTR str);
	//static GUID GetMD5(CFile& File);
	static GUID GetMD5FromFile(LPCTSTR strFilePath);

	//constructor/destructor
	CMD5Checksum();

	void Update(BYTE* Input, unsigned nInputLen);
	void Update(const std::vector<BYTE>& Buf);
	void Update(LPCTSTR str);
	void Update(BYTE Input);
	void Update(DWORD Input);
	void Update(int Input);
	void Update(const StringArray& strings);

	GUID GetFinalChecksum();
