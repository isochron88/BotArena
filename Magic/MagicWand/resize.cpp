...
void CResize::SetResize(const UINT nID, const REALRECT& rrcSizing)
{
	...
	ATLASSERT((CRealRect(0,0,1,1) & pItem->m_rrcSizing) == pItem->m_rrcSizing);
}
...
void CResize::Init()
{
	...
	if (!HasFlag(SZ_NOSIZEICON))
	{
		...
		HBITMAP hPreviousBitmap = m_scSizeIcon.SetBitmap(m_bmSizeIcon);
		if (hPreviousBitmap)
...
BOOL CResize::Defer(HDWP hdwp, CItem* pItem, int dx, int dy)
{
	...
	if (rrcItem != pItem->m_rrcWindow)
	{
		...
		if (::IsWindow(hwnd))
		{
			...
			{
				ATLTRACE("DeferWindowPos failed for ID %i\n", GetDlgCtrlID(hwnd));
				return FALSE;
			}
		}
	}
...
{
	//ATLASSERT(GetStyle() & WS_THICKFRAME); // Needs resizable frame
...
BOOL CResizePropertySheet::OnInitDialog() 
{
	...
	if (CMenu* pSysMenu = GetSystemMenu(FALSE))
	{
		...
		// try to get the strings from the topmost window1
		CWnd* pwndTop = this;
		for (; pwndTop->GetParent(); pwndTop = pwndTop->GetParent());
