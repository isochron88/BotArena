...
NOTES:			Provides an interface to the CMD5Checksum class. 'strFilePath' name should 
				...
				exception.
*****************************************************************************************/
GUID CMD5Checksum::GetMD5FromFile(LPCTSTR strFilePath)
{
	FILE* pFile = NULL;
	...
	BYTE buffer[4096];
	size_t nRead = 0;
	while (nRead = fread(buffer, 1, ARRAY_SIZE(buffer), pFile))
		checksum.Update(buffer, static_cast<unsigned>(nRead));

	fclose(pFile);

	return checksum.GetFinalChecksum();
}
...
*****************************************************************************************/
/*
GUID CMD5Checksum::GetMD5(CFile& File)
{
	try
	{
		...
		return MD5Checksum.GetFinalChecksum();
	}
...
*****************************************************************************************/
GUID CMD5Checksum::GetMD5(BYTE* pBuf, UINT nLength)
{
	//entry invariants
...
GUID CMD5Checksum::GetMD5(const std::vector<BYTE>& Buf)
{
	CMD5Checksum MD5Checksum;		//checksum object	
	MD5Checksum.Update( Buf );

	//finalise the checksum and return it
	return MD5Checksum.GetFinalChecksum();
}

GUID CMD5Checksum::GetMD5(LPCTSTR str)
{
	CMD5Checksum MD5Checksum;		//checksum object	
	MD5Checksum.Update( str );
...
*****************************************************************************************/
void CMD5Checksum::Update( BYTE* Input,	unsigned nInputLen )
{
	if (!nInputLen)
		return;
...
void CMD5Checksum::Update(const std::vector<BYTE>& Buf)
{
	for (unsigned i = 0; i < Buf.size(); ++i)
		Update( const_cast<BYTE*>(&(Buf[i])), sizeof(BYTE) );	
}

void CMD5Checksum::Update(LPCTSTR str)
{
	USES_CONVERSION;

	LPCSTR str2 = T2CA(str);

	Update(reinterpret_cast<BYTE*>(const_cast<LPSTR>(str2)), static_cast<unsigned>(strlen(str2)));
}

void CMD5Checksum::Update(BYTE Input)
{
	Update(&Input, sizeof(Input));
}

void CMD5Checksum::Update(DWORD Input)
{
	Update(reinterpret_cast<BYTE*>(&Input), sizeof(Input));
}

void CMD5Checksum::Update(int Input)
{
	Update(reinterpret_cast<BYTE*>(&Input), sizeof(Input));
}

void CMD5Checksum::Update(const StringArray& strings)
{
	StringArray temp(strings);
	std::sort(temp.begin(), temp.end());
	for (StringArray::const_iterator i = temp.begin(); i != temp.end(); ++i)
		Update(*i);
}

GUID CMD5Checksum::GetFinalChecksum()
{
	//Save number of bits
	BYTE Bits[8];
	DWordToByte( Bits, m_nCount, 8 );
	...
	return *reinterpret_cast<GUID*>(lpszMD5);
}
