                               ...
                               UINT nID, DWORD dwStyle)
{
    ...
    ATLASSERT (!((dwStyle & CBRS_SIZE_FIXED) &&
...
const BOOL CSizingControlBar::IsSideTracking() const
{
    ...
    ATLASSERT(m_bTracking && !IsFloating());
...
void CSizingControlBar::StartTracking(UINT nHitTest)
{
    ...
    else
    {
        ...
        CSizingControlBar* pBar = arrSCBars[nFound +
                                    ...
                                    CRect& rcEdge)
{
    ...
    case HTBOTTOM:
        ...
        break;
    default:
        ATLASSERT(FALSE); // invalid hit test code
    }
    return TRUE;
}
...
UINT CSizingControlBar::GetEdgeHTCode(int nEdge)
{
    ...
    ATLASSERT(FALSE); // invalid edge no
    return HTNOWHERE;
}
...
{
    ASSERT_VALID(this);
    ATLASSERT(GetSafeHwnd()); // must be called after Create()
